describe('Protractor Demo App', function() {
	...
	it('should not signIn to account1', function() {
		...
		//Check for user sign in by giving invalid password and correct email.
		...
		element(by.id('password-login')).clear().then(function() {
			element(by.id('password-login')).sendKeys('abcdefgh1234XXY');
		});
	...
	it('should signIn to account2', function() {
		...
		//Check for user sign in by giving blank epassword and correct email.
		element(by.id('emailAddress-login')).clear().then(function() {
			element(by.id('emailAddress-login')).sendKeys('modata@gmail.com');
		});
		element(by.id('password-login')).clear().then(function() {
			element(by.id('password-login')).sendKeys('');
		});
